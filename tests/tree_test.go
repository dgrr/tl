...
func TestTreeTravel(t *testing.T) {
	...
	rangeCount := 0
	tree.RangeAll(func(node *tl.Tree[string, int]) bool {
		rangeCount++
	...
	tree.RangeLimit(func(node *tl.Tree[string, int]) bool {
		if node.Name() != "d" {
	...
	iter := 0
	tree.RangeLevel(func(node *tl.Tree[string, int]) bool {
		iter++
		if node.Name() != "d" {
	...
	idx := 0
	tree.Range(func(n *tl.Tree[string, int]) bool {
		if n.Name() != order[idx] {
